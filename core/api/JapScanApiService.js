...
class JapScanApiService {
    ...
    parsePages($) {
        ...
        let isSecuredPage = src.startsWith('https://c.japscan.to/clel/')
        let uri = isSecuredPage ? src.replace('https://c.japscan.to/clel/', '').split('/') : src.replace('https://c.japscan.to/lel/', '').split('/')
